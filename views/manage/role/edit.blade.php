...
{!! $navs !!}
<form class="hstui-form hstui-form-horizontal J_ajaxForm" action="{{ route('manage.role.editSave') }}" method="post">
  <input type="hidden" name="id" value="{!! $id !!}" id="thinkwinds_id">
  {!! tw_csrf() !!} 
  <div class="hstui-frame">
    <div class="hstui-frame-title">{!! tw_lang('thinkwinds::manage.role.edit') !!}</div>
    <div class="hstui-frame-content">
      <div class="hstui-form-group hstui-form-group-sm @if($errors->has('name')) hstui-form-error @endif" id="J_form_error_name">
        ...
        <div class="hstui-u-sm-10 hstui-form-input">
          <input type="text" name="name" id="thinkwinds_name" value="{{ tw_value('name', $info) }}" class="hstui-input  hstui-length-3" placeholder="">
                  ...
                  <div class="hstui-u-sm-10">
                    @if(isset($v['url']) && $v['url'])
                      ...
                      @foreach($roleUriDatas[$v['id']] as $r=>$rv)
                        <input name="auths[]" type="checkbox" value="{!! $rv['ename'] !!}" {!! tw_ifCheck(in_array($rv['ename'], $info['auths'])) !!}> {!! $rv['name'] !!}
                      @endforeach
                      @endif
                    @endif
                      ...
                      <div class="hstui-u-sm-10">
                          @if(isset($vs['url']) && $vs['url'])
                            ...
                            @foreach($roleUriDatas[$vs['id']] as $rs=>$rsv)
                              <input name="auths[]" type="checkbox" value="{!! $rsv['ename'] !!}" {!! tw_ifCheck(in_array($rsv['ename'], $info['auths'])) !!}> {!! $rsv['name'] !!}
                            @endforeach
                            @endif
                          @endif
                      </div>
