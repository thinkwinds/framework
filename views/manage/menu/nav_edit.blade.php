...
<form method="post" class="hstui-form hstui-form-horizontal J_ajaxForm" action="{{ route('manage.menu.navEditSave') }}">
    <input type="hidden" name="id" id="thinkwinds_id" value="{!! $id !!}">
    {{ tw_csrf() }}
    <div class="hstui-frame">
      <div class="hstui-frame-content">
        <div class="hstui-form-group hstui-form-group-sm @if($errors->has('parent')) hstui-form-error @endif" id="J_form_error_parent">
          ...
          <div class="hstui-u-sm-10 hstui-form-input">
                <select class="hstui-length-4" name="parent" id="thinkwinds_parent">
                    <option value="0" {!! tw_isSelected('root' == $info['parent']) !!}>{!! tw_lang('thinkwinds::public.top.level') !!}</option>
                    @foreach($menus as $k=>$v)
                        <option value="{!! $v['id'] !!}" {!! tw_isSelected($v['ename'] == $info['parent']) !!}>{!! $v['name'] !!}</option>
                        @if(isset($v['items']) && $v['items'])
                        @foreach($v['items'] as $ks=>$vs)
                        <option value="{!! $vs['id'] !!}" {!! tw_isSelected($vs['ename'] == $info['parents'] && $vs['parent'] == $info['parent']) !!}>  --{!! $vs['name'] !!}</option>
                        @endforeach
                        @endif
                    @endforeach
                </select>
        ...
        <div class="hstui-form-group hstui-form-group-sm @if($errors->has('url')) hstui-form-error @endif" id="J_form_error_url">
          ...
          <div class="hstui-u-sm-10 hstui-form-input">
              <input type="text" name="url" id="thinkwinds_url" value="{{ tw_value('url', $info) }}" class="hstui-input hstui-length-4">
        ...
        <div class="hstui-form-group hstui-form-group-sm @if($errors->has('icon')) hstui-form-error @endif" id="J_form_error_icon">
          ...
          <div class="hstui-u-sm-10 hstui-form-input">
              <input type="text" name="icon" id="thinkwinds_icon" value="{{ tw_value('icon', $info) }}" class="hstui-input hstui-length-4">
