...
<body>
  <form class="hstui-form hstui-form-horizontal J_ajaxForm" action="{{ route('manage.block.editSave', ['module'=>$module]) }}" method="post">
    {!! tw_csrf() !!}
    ...
    <div class="hstui-frame">
      <div class="hstui-frame-content">
        ...
        <div class="hstui-form-group hstui-form-group-sm @if($errors->has('type')) hstui-form-error @endif" id="J_form_error_type">
          ...
          <div class="hstui-u-sm-10 hstui-form-input">
             <input type="radio" onchange="clickType('text')" name="type" value="text" {{ tw_ifCheck($info['type'] == 'text') }}>
             <label>文本</label>
             <input type="radio" onchange="clickType('html')" name="type" value="html" {{ tw_ifCheck($info['type'] == 'html') }}> 
             <label>html</label>
             <input type="radio" onchange="clickType('image')" name="type" value="image" {{ tw_ifCheck($info['type'] == 'image') }}> 
             <label>图片</label>
        ...
        <div class="hstui-form-group hstui-form-group-sm @if($info['type'] != 'image') hstui-hide @endif @if($errors->has('link')) hstui-form-error @endif" id="J_form_error_link">
          ...
          <div class="hstui-u-sm-10 hstui-form-input">
              <input type="text" name="link" id="thinkwinds_link" value="{{ tw_value('link', $info['image']) }}" class="hstui-input hstui-length-6">
        ...
        <div class="hstui-form-group hstui-form-group-sm @if($info['type'] != 'html') hstui-hide @endif @if($errors->has('contentv')) hstui-form-error @endif" id="J_form_error_contentv">
          <label class="hstui-u-sm-2 hstui-form-label">{{ tw_lang('thinkwinds::public.content') }}</label>
          <div class="hstui-u-sm-10 hstui-form-input">
              <textarea class="hstui-textarea" style="height: 420px; width: 100%;" name="contentv" id="thinkwinds_contentv">{{ tw_value('content', $info) }}</textarea>
          </div>
        </div>
      </div>
    </div>
