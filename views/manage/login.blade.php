...
<div class="hstui-content">
  <div class="login-page">
    <div class="login-form">
      ...
      <form class="hstui-form hstui-form-horizontal" action="{!! route('manage.auth.dologin') !!}" method="post">
      {!! tw_csrf() !!}
        <div class="hstui-form-group hstui-form-group-sm @if($errors->has('username')) hstui-form-error @endif">
          <div class="hstui-input-icon hstui-icon-lf @if($errors->has('username')) hstui-field-error @endif">
            ...
            <input class="hstui-input" id="userName" name="username" value="{{ tw_value('username') }}" placeholder="{{ tw_lang('thinkwinds::public.enter.one.username') }}" />
          </div>
        </div>
        ...
        <button type="submit" class="hstui-button hstui-button-primary hstui-button-block" data-button-content="{{ tw_lang('thinkwinds::public.login.loading') }}">{{ tw_lang('thinkwinds::public.login') }}</button>
        @if (count($errors) > 0)
...
Hstui.use('jquery','common',function() {
  if(window.parent.location.href !== '{!! route('manage.auth.login') !!}') 
  {
    window.parent.location.href = '{!! route('manage.auth.login') !!}';
  }
