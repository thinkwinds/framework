...
class Date extends FieldAbs 
{
    ...
    {
		parent::__construct();
		$this->name = tw_lang('thinwinds::fields.date'); // 字段名称
	...
	public function option($option) 
	{
		...
		return '
			<div class="hstui-form-group hstui-form-group-sm">
	          <label class="hstui-u-sm-2 hstui-form-label">'.tw_lang('thinwinds::public.width').'</label>
	          <div class="hstui-u-sm-10 hstui-form-input">
	              <input type="text" name="setting[option][width]" value="'.$option['width'].'" class="hstui-input hstui-length-5">
	            <div class="hstui-form-input-tips" id="J_form_tips_name" data-tips="'.tw_lang('thinwinds::manage.fields.type.width.tips').'">'.tw_lang('thinwinds::manage.fields.type.width.tips').'</div>
	          </div>
	        </div>
			<div class="hstui-form-group hstui-form-group-sm">
	          <label class="hstui-u-sm-2 hstui-form-label">'.tw_lang('thinwinds::fields.date.geshi').'</label>
	          <div class="hstui-u-sm-10 hstui-form-input">
	              <input type="text" name="setting[option][format]" value="'.$option['format'].'" class="hstui-input hstui-length-5">
	            <div class="hstui-form-input-tips" id="J_form_tips_name" data-tips="'.tw_lang('thinwinds::fields.date.geshi.tips').'">'.tw_lang('thinwinds::fields.date.geshi.tips').'</div>
	          </div>
	        </div>
			<div class="hstui-form-group hstui-form-group-sm">
	          <label class="hstui-u-sm-2 hstui-form-label">'.tw_lang('thinwinds::public.default.value').'</label>
	          <div class="hstui-u-sm-10 hstui-form-input">
	              <input type="text" name="setting[option][value]" value="'.$option['value'].'" class="hstui-input hstui-length-5 J_date">
	            <div class="hstui-form-input-tips" id="J_form_tips_name" data-tips="'.tw_lang('thinwinds::manage.fields.default.value.001.tips').'">'.tw_lang('thinwinds::manage.fields.default.value.001.tips').'</div>
	          </div>
	        </div>
                <script>
                //日期+时间选择器
	...
	public function input($cname, $name, $cfg, $value = NULL, $id = 0) 
	{
		...
		$str = '';
		$str.= '<input type="text" class="hstui-input J_date" value="'.$show.'" name="'.$name.'" id="thinkwinds_'.$name.'" ' . $width . $disabled . $attr.$required.' />';
        if ($name == 'updatetime') 
        {
